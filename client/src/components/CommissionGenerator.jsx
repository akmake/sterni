...
import { AlertTriangle, Save, Filter, CheckCircle2, Pencil, Database, Percent, FileDown } from 'lucide-react';
...
export default function CommissionGenerator({ onReportGenerated }) {
    ...
    const handleExportExcel = () => {
        if (processedRows.length === 0) return toast.error("אין נתונים לייצוא");

        const exportData = processedRows.map(row => {
            const diff = Math.round((row.finalInvoiceAmount || 0) - (row.expectedWithVat || 0));
            const status = row.manualFix ? 'תוקן ידנית' : row.colorStatus === 'green' ? 'תקין' : row.colorStatus === 'red' ? 'חסר' : 'עודף';
            return {
                'הזמנה': row.masterId,
                'אורח': row.guestName,
                'טלפון': row.customerPhone || '',
                'ת. הגעה': row.arrivalDate ? format(row.arrivalDate, 'dd/MM/yyyy') : '-',
                'נציג': row.clerk + (row.isSplit ? ` (${row.splitRole === 'creator' ? 'יוצר' : 'סוגר'})` : ''),
                'חשבונית': row.finalInvNum || '',
                'ללא מע"מ': row.totalOrderPrice,
                'צפוי כולל': parseFloat(row.expectedWithVat.toFixed(2)),
                'בפועל': parseFloat(row.finalInvoiceAmount.toFixed(2)),
                'פער': diff,
                'עמלה': parseFloat(row.commissionToPay.toFixed(2)),
                'אחוז עמלה %': parseFloat(row.commissionRate.toFixed(1)),
                'סטטוס': status,
            };
        });
        ...
        const fileName = `עמלות_${format(new Date(), 'dd-MM-yyyy_HH-mm')}.xlsx`;
        XLSX.writeFile(wb, fileName);
        toast.success(`${processedRows.length} שורות יוצאו לקובץ ${fileName}`);
    };
    ...
    const handleLoadFromDB = async () => {
        ...
        try {
            ...
            const convertedData = relevantOrders.map(order => ({
                ...
                [RES_COL_PHONE]: order.customerPhone,
    ...
    const handleAnalyze = () => {
        ...
        reservationsData.forEach(row => {
            ...
            let customerPhone = cleanStr(row[RES_COL_PHONE]);
                                    ...
                                    <tbody className="divide-y">
                                        {visibleRows.map(row => (
                                            <tr key={row.uniqueKey} className={`hover:bg-slate-50 transition-colors ${selectedRows.has(row.uniqueKey) ? 'bg-green-50' : ''}`}>
                                                ...
                                                {/* עמודת הפער החדשה */}
                                                <td className="p-3 text-center" style={{ direction: 'ltr', fontWeight: 'bold' }}>
                                                    {(() => {
                                                        const diff = (row.finalInvoiceAmount || 0) - (row.expectedWithVat || 0);
                                                        const color = diff > 0 ? 'text-green-600' : diff < 0 ? 'text-red-600' : 'text-gray-400';
                                                        const sign = diff > 0 ? '+' : ''; 
                                                        return <span className={color}>{sign}{Math.round(diff).toLocaleString()}</span>;
                                                    })()}
                                                </td>
