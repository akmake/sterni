...
const QuoteManager = ({ currentData, onLoadData }) => {
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      // תיקון 1: הוספת withCredentials
      const res = await axios.get('/api/quotes', { withCredentials: true }); 
      setSavedFiles(res.data.data.quotes);
      setLoading(false);
    } catch (err) {
      console.error(err);
      setMessage('שגיאה בטעינת רשימה - אולי לא מחובר?');
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      // תיקון 2: הוספת withCredentials (פרמטר שלישי ב-post)
      ...
      console.error(err);
      toast.error('שגיאה בשמירה (אולי השם כבר קיים?)');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLoad = async (name) => {
    try {
      setLoading(true);
      // תיקון 3: הוספת withCredentials
      const res = await axios.get(`/api/quotes/${name}`, { withCredentials: true });
      onLoadData(res.data.data.quote.content); 
      setIsOpen(false);
      setMode(null);
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (e, name) => {
    ...
    try {
      // תיקון 4: הוספת withCredentials
      await axios.delete(`/api/quotes/${name}`, { withCredentials: true });
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {new Date(file.updatedAt).toLocaleDateString('he-IL')} {new Date(file.updatedAt).toLocaleTimeString('he-IL', {hour: '2-digit', minute:'2-digit'})}
                      </div>
                    </div>
                    <button 
