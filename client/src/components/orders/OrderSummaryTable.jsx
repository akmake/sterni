...
import { SlidersHorizontal, Moon, Trash2 } from 'lucide-react';
...
export function OrderSummaryTable({ rooms, priceLists, roomTypes, numberOfNights, onRemoveRoom, onUpdateRoom }) {
  ...
  const handlePriceListChange = (roomIndex, priceListName) => {
    ...
    roomToUpdate.price_list_names = currentSelection.includes(priceListName)
  ...
  const recalculateAndSave = (index, updatedRoom) => {
    ...
    const newPrice = calculateRoomTotalPrice(
      updatedRoom, priceLists, updatedRoom.price_list_names, numberOfNights, supplement
    );
  ...
  const numberInputClass = "w-10 h-8 border border-gray-200 rounded text-center text-sm focus:outline-none focus:border-blue-400 bg-white";
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-2 py-2 text-right text-xs font-medium uppercase min-w-[150px]">הערות</th>
            ...
            {rooms.map((room, index) => (
              <tr key={index}>
                <td className="px-2 py-2">
                  <Select value={room.roomTypeId || ''} onValueChange={(val) => handleRoomTypeChange(index, val)}>
                    <SelectTrigger className="h-8 text-xs w-full bg-white border-gray-200">
                      <SelectValue placeholder={room.roomType || 'רגיל'} />
                    </SelectTrigger>
                    <SelectContent>
                      {roomTypes.map(rt => (
                        <SelectItem key={rt._id} value={rt._id} className="text-xs">{rt.name}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </td>
                <td className="px-1 py-2 text-center">
                  <input type="number" min="0" className={numberInputClass} value={room.adults > 0 ? room.adults : ''} onChange={(e) => handleCellChange(index, 'adults', e.target.value)} />
                </td>
                <td className="px-1 py-2 text-center">
                  <input type="number" min="0" className={numberInputClass} value={room.teens > 0 ? room.teens : ''} onChange={(e) => handleCellChange(index, 'teens', e.target.value)} />
                </td>
                <td className="px-1 py-2 text-center">
                  <input type="number" min="0" className={numberInputClass} value={room.children > 0 ? room.children : ''} onChange={(e) => handleCellChange(index, 'children', e.target.value)} />
                </td>
                <td className="px-1 py-2 text-center">
                  <input type="number" min="0" className={numberInputClass} value={room.babies > 0 ? room.babies : ''} onChange={(e) => handleCellChange(index, 'babies', e.target.value)} />
                </td>
                <td className="px-2 py-2 text-xs">
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" size="sm" className="w-full justify-between h-8 text-xs px-2 bg-white border-gray-200">
                ...
                <td className="px-2 py-2">
                  <Input type="text" value={room.notes || ''} onChange={(e) => handleCellChange(index, 'notes', e.target.value)} className="h-8 text-xs min-w-[120px]" placeholder="הערה..." />
                </td>
