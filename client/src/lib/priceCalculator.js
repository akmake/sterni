// חישוב מחירים - מסונכרן עם ziporiclass
...
export function calculateRoomTotalPrice(room, allPriceLists, selectedNames, nights = 1, roomSupplementPerNight = 0) {
  ...
  // תמיכה גם במפה (object) וגם במערך
  const priceListsMap = Array.isArray(allPriceLists)
    ? Object.fromEntries(allPriceLists.map(pl => [pl.name, pl]))
    : allPriceLists;

  let totalPackageBasePrice = 0;
  for (const name of selectedNames) {
    const priceList = priceListsMap[name];
    if (priceList) {
