...
import { ArrowRight, Download, Send, User, LoaderCircle, Mail, Zap, Printer } from 'lucide-react';
...
export default function HotelQuotePage() {
  ...
  if (isLoading) return <div className="h-screen flex items-center justify-center"><LoaderCircle className="animate-spin h-10 w-10 text-blue-600" /></div>;
    ...
    <div className="min-h-screen bg-gray-200 pb-10 font-sans">
      ...
      <header className="bg-white border-b px-6 py-4 flex justify-between items-center sticky top-0 z-50 shadow-sm no-print">
        ...
        <div className="flex gap-2">
          <Button onClick={() => window.print()} variant="outline" className="gap-2 border-slate-300 text-slate-700">
            <Printer className="h-4 w-4" /><span className="hidden sm:inline">הדפס</span>
          </Button>
          <Button onClick={handleDownloadPDF} disabled={isDownloading} className="bg-blue-600 hover:bg-blue-700 text-white gap-2">
          ...
          {order.customerEmail && (
            <Button onClick={handleQuickSend} disabled={isSendingEmail || isDownloading} className="gap-2 bg-green-600 hover:bg-green-700 text-white">
              ...
              <>
                ...
                <div className="bg-gray-50 rounded-xl p-6 mb-10 border border-gray-100">
                  ...
                  <div className="grid grid-cols-2 gap-y-4 gap-x-8">
                    ...
                    {order.customerEmail && <div><span className="text-xs text-gray-500 uppercase tracking-wider block">אימייל</span><p className="text-base font-medium">{order.customerEmail}</p></div>}
                      ...
                      if (item._type === 'header') {
                        return (
                          <tr key={`header-${idx}`} className="bg-slate-50">
                            <td colSpan="4" className="py-2 px-4 font-bold text-slate-700 text-sm border-y">{item.name}</td>
                          </tr>
                        );
                      }
                      return (
                        <tr key={idx}>
                          ...
                          <td className="py-4 px-4 align-top">
                            ...
                            {item._type === 'room' && item.description && (
                              <p className="text-xs text-gray-500 mt-1 bg-gray-100 inline-block px-2 py-0.5 rounded">{item.description}</p>
                            )}
                            {item._type === 'room' && item.roomNote && (
                              <p className="text-xs text-blue-700 mt-1 italic bg-blue-50 px-2 py-1 rounded border border-blue-100/50 inline-block mr-2">הערה: {item.roomNote}</p>
                            )}
                            {item._type === 'extra' && <p className="text-xs text-blue-600 mt-1">תוספת</p>}
                          </td>
                          <td className="py-4 px-4 align-top">
                            {item._type === 'room' ? (
                              ...
                              <div className="text-gray-700 font-medium">כמות: {item.details.quantity}</div>
                            )}
                          </td>
                          <td className="py-4 px-4 align-top text-left font-medium text-slate-900">{item.price.toLocaleString()} ₪</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div className="mt-auto">
                {order.notes && (
                  <div className="mb-6 border-t border-dashed border-gray-300 pt-6">
                    <h4 className="font-bold text-sm text-slate-800 mb-3">הערות ודגשים להזמנה:</h4>
                    <div className="text-sm text-slate-700 leading-relaxed bg-yellow-50/50 p-4 rounded-lg border border-yellow-100/50" style={{ whiteSpace: 'pre-wrap' }}>
                      {order.notes}
                    </div>
                  </div>
                )}
                <div className="border-t-2 border-slate-100 pt-6 mb-8">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-4xl font-extrabold text-blue-600">{order.total_price.toLocaleString()} ₪</p>
                    </div>
                  </div>
                </div>
