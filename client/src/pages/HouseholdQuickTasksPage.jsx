...
const HouseholdQuickTasksPage = () => {
  ...
  const convertToProject = async (id) => {
    if (!window.confirm('להפוך מטלה זו לפרויקט משק בית חדש? המטלה תוסר מהרשימה.')) return;
    try {
      const task = tasks.find(t => t._id === id);
      // Create a household project of type 'simple', then delete the task
      const res = await api.post('/household-projects', {
      ...
      await api.delete(`/tasks/${id}`);
      setTasks(prev => prev.filter(t => t._id !== id));
      navigate(`/household/projects/${res.data._id}`);
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 pt-4">
          ...
          <div className="flex justify-between items-end">
            <h1 className="text-4xl font-bold tracking-tight">מטלות מהירות</h1>
...
export default HouseholdQuickTasksPage;
