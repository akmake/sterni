...
const createHotel = (name) => api.post('/hotel-data/hotels', { name });
const deleteHotel = (id) => api.delete(`/hotel-data/hotels/${id}`);
const updateMasterChecklist = ({ id, checklists }) => api.put(`/hotel-data/hotels/${id}/checklist`, { checklists });

const fetchRoomTypes = async (hotelId) => (await api.get(`/hotel-data/room-types/by-hotel/${hotelId}`)).data;
const createRoomType = (data) => api.post('/hotel-data/room-types', data);
const deleteRoomType = (id) => api.delete(`/hotel-data/room-types/${id}`);
...
function MasterChecklistDialog({ hotel, isOpen, onClose }) {
    ...
    const [checklists, setChecklists] = useState({
        departure: hotel.checklists?.departure || [],
            ...
            <DialogContent className="max-w-2xl min-h-[600px] flex flex-col">
                ...
                <div className="w-full flex-1 flex flex-col">
                    <TabsList className="grid w-full grid-cols-3">
                        <TabsTrigger value="departure" onClick={() => setActiveTab('departure')} data-state={activeTab === 'departure' ? 'active' : ''}>עזיבה / ניקיון יסודי</TabsTrigger>
                        <TabsTrigger value="stayover" onClick={() => setActiveTab('stayover')} data-state={activeTab === 'stayover' ? 'active' : ''}>שהייה (ניקיון שוטף)</TabsTrigger>
                        <TabsTrigger value="arrival" onClick={() => setActiveTab('arrival')} data-state={activeTab === 'arrival' ? 'active' : ''}>כניסה (ביקורת)</TabsTrigger>
                    </TabsList>
